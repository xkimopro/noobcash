...
class EventListeningThread(Thread):
    ...
    def run(self, *args, **kwargs):
        i = 0
        while True:
            if len(self.cached_messages) > 0 and not self.node.mining and not self.node.resolving_confilct: 
            ...
            if conflict_id is not None:
                    if not self.node.resolving_confilct:
                        ...
                        print("Received request to vote for conflict resolution by node" + str(conflict_id) + " while in conflict mode")
